...
def _best_valid(candidates: Sequence[Candidate]) -> Optional[Candidate]:
...
def solve(
    ...
    draft_tune = _score_candidate(
        _ask_model(
            ...
            _build_repair_prompt(
                ...
                original_code,
                instruction_correct + "\n\n" + instruction_clean,
                extra_rules="Prefer compact, tidy code when correctness is unchanged.",
            ),
        ),
        "TUNE",
        original_code,
    ...
    pool = _dedupe_candidates([original_candidate, draft_base, draft_tune])
    best_now = _best_valid(pool) or original_candidate
    if best_now.valid and best_now.label != "ORIGINAL" and best_now.score >= original_candidate.score + 10.0 and risk <= 2:
        verified = _score_candidate(
            _ask_model(
                "verify_fast",
                client_base,
                model,
                _build_verify_prompt(spec, task_description, original_code, best_now.code),
            ),
            "VERIFIED_FAST",
            original_code,
            original_meta,
        ...
        return verified.code if verified.valid else best_now.code

    refinements = []
    for candidate in _rank_candidates(pool)[:3]:
        if not candidate.valid or candidate.label == "ORIGINAL":
            continue
        critic_client = client_tune if candidate.source == "draft_base" else client_base
        revise_client = client_base if critic_client is client_tune else client_tune
        critique = _ask_model(
            f"critique_{candidate.label}",
            critic_client,
            model,
            _build_critic_prompt(spec, task_description, original_code, candidate.code),
        )
        refined = _score_candidate(
            _ask_model(
                f"revise_{candidate.label}",
                revise_client,
                model,
            ...
            f"{candidate.label}_R",
            original_code,
            original_meta,
            original_complexity,
            cache,
            source=f"revise_{candidate.label}",
        )
        refinements.append(refined)

    final_pool = _rank_candidates(_dedupe_candidates(pool + refinements))
    champion = final_pool[0] if final_pool else original_candidate
    runner_up = final_pool[1] if len(final_pool) > 1 else original_candidate
    if champion.valid and runner_up.valid and champion.label != runner_up.label:
        champion = _pairwise_pick(spec, original_code, champion, runner_up, client_base, model)

    if not champion.valid or champion.label == "ORIGINAL":
    ...
    verified = _score_candidate(
        _ask_model(
            ...
            client_base,
            model,
            _build_verify_prompt(spec, task_description, original_code, champion.code),
        ),
        "VERIFIED",
        original_code,
        original_meta,
    ...
    return champion.code if champion.valid else original_candidate.code
