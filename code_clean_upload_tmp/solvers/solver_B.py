...
def solve(
    ...
    quick_tune = _score_candidate(
        _ask_model(
            ...
            _build_repair_prompt(
                "FAST_TUNE_AGENT",
                "INTENT: quick safe cleanup\nINPUTS: existing code\nOUTPUTS: modified code\nMUST_PRESERVE: public interface\nALLOWED_CHANGES: minimal safe edits\nAMBIGUITIES: keep unchanged",
                task_description,
                original_code,
                instruction_correct + "\n\n" + instruction_clean,
                extra_rules="Make the smallest safe edit set. Prefer unchanged code over risky edits.",
            ),
        ),
        "QUICK_TUNE",
        original_code,
    ...
    if (
        quick_tune.valid
        and quick_tune.score >= original_candidate.score + 8.0
        and (original_complexity is None or quick_tune.complexity is None or quick_tune.complexity <= original_complexity)
        and risk <= 2
    ):
        verified = _score_candidate(
            _ask_model(
                ...
                client_base,
                model,
                _build_verify_prompt("INTENT: quick safe cleanup", task_description, original_code, quick_tune.code),
            ),
            "QUICK_VERIFIED",
            original_code,
            original_meta,
    ...
    correct_base = _score_candidate(
        _ask_model(
            ...
            _build_repair_prompt(
                ...
                original_code,
                instruction_correct,
                extra_rules="Prefer semantic correctness over stylistic cleanup.",
            ),
        ),
        "CORRECT_BASE",
        original_code,
    ...
    clean_tune = quick_tune
    if not quick_tune.valid or risk >= 3:
        clean_tune = _score_candidate(
            _ask_model(
                "clean_tune",
                client_tune,
                model,
                _build_repair_prompt(
                    ...
                    original_code,
                    instruction_correct + "\n\n" + instruction_clean,
                    extra_rules="Prefer smaller diffs, early returns, and lower branching.",
                ),
            ),
            "CLEAN_TUNE",
            original_code,
    ...
    pool = _rank_candidates(_dedupe_candidates([original_candidate, quick_tune, correct_base, clean_tune]))
    viable = [candidate for candidate in pool if candidate.valid]
    if not viable:
        return original_candidate.code

    anchor = viable[0]
    alt = viable[1] if len(viable) > 1 else original_candidate
    if anchor.label != "ORIGINAL":
        critic_client = client_tune if anchor.source in {"correct_base", "quick_verify"} else client_base
        revise_client = client_base if critic_client is client_tune else client_tune
        critique = _ask_model(
            "anchor_critique",
            critic_client,
            model,
            _build_critic_prompt(spec, task_description, original_code, anchor.code),
        )
        merged = _score_candidate(
            _ask_model(
                "anchor_revise",
                revise_client,
                model,
            ...
            original_complexity,
            cache,
            source="anchor_revise",
        )
        pool = _rank_candidates(_dedupe_candidates(pool + [merged]))
        viable = [candidate for candidate in pool if candidate.valid]
        anchor = viable[0]
        alt = viable[1] if len(viable) > 1 else original_candidate

    winner = anchor if alt.label == anchor.label else _pairwise_pick(spec, original_code, anchor, alt, client_base, model)
