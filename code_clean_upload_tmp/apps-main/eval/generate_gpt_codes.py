...
def generate_prompt(args, test_case, prompt, solutions, tokenizer=None, starter_code=None):
    _input = "\nQUESTION:\n"
    data = prompt
    _input += data
    ...
    if args.peeking > 0.0 and tokenizer:
...
def load_dataset(path,split):
    ...
    split_="test"
    problems = my_utils.load_apps_per_folder(
...
def main(args):
    ...
    if(1):
        ...
        problems = my_utils.load_apps_per_folder(
            split=args.split,                # "test" 或 "train"
            root_dir="/home/wangbn/APPS"     # 你的实际路径
        )
    #load_dataset("/home/apps")
    ...
    for index, problem in enumerate(tqdm(problems)):
        ...
        prompt_text, sample_sol = generate_prompt(args, test_case, prompt, solutions, tokenizer=None, starter_code=starter_code)
        if args.debug:
            print("PROMPT_TEXT:")
            print(prompt_text)
        ...
        try:
            if(0):
                ...
                output_str=my_utils.infer_once(prompt_text)
                #output_str="prompt_text={} !!".format(prompt_text)
