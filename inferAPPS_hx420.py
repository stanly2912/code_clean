...
# ==========================================
# 三个推理函数 (API, Local, MAS)
# ==========================================

def generate_online(client, model_name, messages, temperature, k_samples, task_id):
    """Online 模式：利用 API 的 n 参数一次性并行获取多个样本，替代 for 循环"""
    completions_list = []
    try:
        ...
        for choice in response.choices:
            clean_code = get_clean_code(choice.message.content)
...
def generate_local(model, tokenizer, messages, temperature, k_samples, task_id):
    """Local 模式：使用集束搜索 (num_beams & num_return_sequences) 替代 for 循环"""
    completions_list = []
    try:
        ...
        generated_ids = [
            output_ids[input_len:] for output_ids in generated_ids_batch
        ]

        responses = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
        for resp in responses:
            clean_code = get_clean_code(resp)
            print("clean_code=",clean_code)
            completions_list.append(clean_code)
...
def generate_mas(client_base, client_tune, messages, question_text, original_codes, task_id, sample_idx, args):
    """MAS 模式：严令禁止修改，原版照抄"""
    clean_code = ""
    try:
        original_code=""
...
def main():
    ...
    for split_name in splits:
        ...
        for task_id in tqdm(all_ids, desc=f"{split_name} 进度"):
            ...
            # ========================================
            if MODE == "online":
                completions_list = generate_online(client, ONLINE_MODEL, messages, TEMPERATURE, K_SAMPLES, task_id)
                
            elif MODE == "local":
                completions_list = generate_local(model, tokenizer, messages, TEMPERATURE, K_SAMPLES, task_id)
                
            elif MODE == "MAS":
                # MAS 模式由于特殊的机制，继续保留它的 for 循环
                for sample_idx in range(K_SAMPLES):
                    time.sleep(0.1)
                    print("sample_idx=", sample_idx)
                    clean_code = generate_mas(client_base, client_tune, messages, question_text, original_codes, task_id, sample_idx, args)
                    print("clean_code=", clean_code)
                    completions_list.append(clean_code)

            # 如果某些错误导致没生够 K 个，用空字符串补齐，防止后续评估断档报错
            while len(completions_list) < K_SAMPLES:
            ...
            if completions_list:
                result = {
                    ...
                    "completions": completions_list  # 包含 K 个元素的数组
                }
