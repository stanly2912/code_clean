#!/bin/bash
set -euo pipefail

# ==========================================
# 0. 通用函数
# ==========================================
timestamp() {
    date '+%F %T'
}

log() {
    echo "[$(timestamp)] $*"
}

cleanup() {
    log "清理后台 API 进程..."
    [[ -n "${API_PID:-}" ]] && kill "${API_PID}" 2>/dev/null || true
    [[ -n "${API_BASE_PID:-}" ]] && kill "${API_BASE_PID}" 2>/dev/null || true
}
trap cleanup EXIT

wait_for_port() {
    local host="$1"
    local port="$2"
    local pid="${3:-}"
    local service_name="$4"
    local timeout="${5:-1800}"   # 默认最多等 30 分钟
    local interval="${6:-2}"     # 每 2 秒检查一次

    local start_ts
    local now_ts
    local elapsed

    start_ts=$(date +%s)
    log "等待 ${service_name} 就绪: ${host}:${port} (超时 ${timeout}s)"

    while true; do
        # 检查端口是否已开启
        if (echo > "/dev/tcp/${host}/${port}") >/dev/null 2>&1; then
            now_ts=$(date +%s)
            elapsed=$((now_ts - start_ts))
            log "${service_name} 已就绪，端口 ${port} 已开启，耗时 ${elapsed}s"
            return 0
        fi

        # 检查进程是否意外退出
        if [[ -n "${pid}" ]] && ! kill -0 "${pid}" 2>/dev/null; then
            log "错误: ${service_name} 进程已退出，启动失败。"
            return 1
        fi

        now_ts=$(date +%s)
        elapsed=$((now_ts - start_ts))

        # 超时退出
        if (( elapsed >= timeout )); then
            log "错误: 等待 ${service_name} 超时 (${timeout}s)，端口 ${port} 仍未开启。"
            return 1
        fi

        # 每 10 秒打印一次等待状态
        if (( elapsed == 0 || elapsed % 10 == 0 )); then
            log "${service_name} 仍在启动中... 已等待 ${elapsed}s"
        fi

        sleep "${interval}"
    done
}

# ==========================================
# 1. 环境清理配置
# ==========================================
PROJECT_DIR="./" #TODO
PYTHON_SCRIPT="${PROJECT_DIR}/inferAPPS_wbn.py"

BASE_RESULT_DIR="${PROJECT_DIR}infer_results_hx/APPS_results"
APPS_PATH="/home/wangbn/code_clean/APPS/test"

# 测试题目数量 (统一配置，方便修改)
NUM_INTRO=1
NUM_INTERVIEW=1
NUM_COMP=1


# ==========================================
# 3. 模型配置列表 (格式: 模式|标识名|模型路径或名称|额外API_URL)
# ==========================================
MODELS=(

    "local|QW_coder_7B_Base|/home/data/wangbn/7B_model/Qwen2.5-Coder-7b-Base|"


    "online|QW_coder_7B_instruct_jiekou420|qwen/qwen2.5-7b-instruct|https://api.jiekou.ai/openai|sk_VrZ4jhQDLWUK2lyQj40u5yr6p5Uq1lLpnQ4Cxh_BnUY"
    #"online|QW_coder_7B_instruct_sillcon_1954|Qwen/Qwen2.5-7B-Instruct|https://api.siliconflow.cn/v1|sk-dpgpkzrralqslyeibhmecrxuphylarytbstgsqbdrnoyykbb"
    "online|mis7b_API_420|open-mistral-7b|https://api.agicto.cn/v1|sk-88UT5OLYRw6so66EliV7rNFI4Y9oblR1Lns3dKNjwXABVtk7"
    "online|qwen3-8B_API_420|qwen3-vl-8b-instruct|https://api.agicto.cn/v1|sk-88UT5OLYRw6so66EliV7rNFI4Y9oblR1Lns3dKNjwXABVtk7"


    "online|hunyuan-7b_420|tencent/Hunyuan-MT-7B|https://api.siliconflow.cn/v1|sk-dpgpkzrralqslyeibhmecrxuphylarytbstgsqbdrnoyykbb"

    "online|DSR1-7b_420|deepseek-ai/DeepSeek-R1-Distill-Qwen-7B|https://api.siliconflow.cn/v1|sk-dpgpkzrralqslyeibhmecrxuphylarytbstgsqbdrnoyykbb"

    "online|internlm2.5_420|internlm/internlm2_5-7b-chat|https://api.siliconflow.cn/v1|sk-dpgpkzrralqslyeibhmecrxuphylarytbstgsqbdrnoyykbb"
   
   
    # --- MAS 模式 ---

    #"MAS|QWcode25_MAS|inferQWcode25.yaml|"
    
    #"local|QW_coder_7B_instruct|/home/wangbn/7B_model/Qwen2_5-Coder-7B-instruct|"


    #"local|QW_coder_7B_Base419|/home/data/wangbn/7B_model/Qwen2.5-Coder-7b-Base||"
    # --- Online API 模型 ---
   # "online|QW_coder_7B_instruct_jiekou420|qwen/qwen2.5-7b-instruct|https://api.jiekou.ai/openai|sk_VrZ4jhQDLWUK2lyQj40u5yr6p5Uq1lLpnQ4Cxh_BnUY|"
    #"online|QW_coder_7B_instruct_sillcon420|Qwen/Qwen2.5-7B-Instruct|https://api.siliconflow.cn/v1|sk-dpgpkzrralqslyeibhmecrxuphylarytbstgsqbdrnoyykbb|"


    #"online|mis7b_API419|open-mistral-7b|https://api.agicto.cn/v1|sk-88UT5OLYRw6so66EliV7rNFI4Y9oblR1Lns3dKNjwXABVtk7"
    #"online|qwen3-8B_API419|qwen3-vl-8b-instruct|https://api.agicto.cn/v1|sk-88UT5OLYRw6so66EliV7rNFI4Y9oblR1Lns3dKNjwXABVtk7"
    
    #"local|LMA_7b_base420|/home/data/wangbn/7B_model/CodeLlama-7b-hf||"
    #"local|dolphin-2.6-mistral-7b-dpo419|/home/data/wangbn/7B_model/dolphin-2.6-mistral-7b-dpo||"
    #"local|deepseek-coder-6.7b-instruct419|/home/data/wangbn/7B_model/deepseek-coder-6.7b-instruct||"

    #"online|hunyuan-7b419|tencent/Hunyuan-MT-7B|https://api.siliconflow.cn/v1|sk-dpgpkzrralqslyeibhmecrxuphylarytbstgsqbdrnoyykbb"

    # --- 本地模型 ---
    #"local|LMA_7B_Base|/home/wangbn/7B_model/CodeLlama-7b-hf||"
   
    #"local|DS_7B_Base|/home/wangbn/7B_model/deepseek-llm-7b-base||"
)

# ==========================================
# 4. 执行循环
# ==========================================
for config in "${MODELS[@]}"; do
    IFS='|' read -r MODE MODEL_NAME MODEL_ARG API_URL API_KEY <<< "${config}"

    log "================================================="
    log "启动测试: [${MODEL_NAME}] 模式: [${MODE}]"
    log "================================================="

    OUT_DIR="${BASE_RESULT_DIR}/${MODEL_NAME}"

    CMD=(
        python "${PYTHON_SCRIPT}"
        --mode "${MODE}"
        --output_path "${OUT_DIR}"
        --introductory "${NUM_INTRO}"
        --interview "${NUM_INTERVIEW}"
        --competition "${NUM_COMP}"
        --apps_path "${APPS_PATH}"
    )

    if [[ "${MODE}" == "MAS" ]]; then

            # ==========================================
        # 2. 启动 6001 Base 模型
        # ==========================================
        log "启动 Base 模型服务 (Port 6001)..."
        cd /home/wangbn/LLaMA-Factory || exit 1

        API_BASE_LOG="/home/wangbn/api6001.log"
        API_PORT=6001 CUDA_VISIBLE_DEVICES=1 nohup \
            llamafactory-cli api /home/wangbn/code_clean/infer_scripts/inferQWcode.yaml \
            > "${API_BASE_LOG}" 2>&1 &
        API_BASE_PID=$!

        log "Base 模型 PID: ${API_BASE_PID}"
        wait_for_port "127.0.0.1" 6001 "${API_BASE_PID}" "Base 模型服务"



        cd /home/wangbn/LLaMA-Factory || exit 1
        API_TUNE_LOG="/home/wangbn/api7001_${MODEL_NAME}.log"

        log "启动 MAS Tune API (Port 7001)，配置: ${MODEL_ARG}"
        API_PORT=7001 CUDA_VISIBLE_DEVICES=1 nohup \
            llamafactory-cli api "/home/wangbn/code_clean/infer_scripts/${MODEL_ARG}" \
            > "${API_TUNE_LOG}" 2>&1 &
        API_PID=$!

        log "MAS Tune API PID: ${API_PID}"
        wait_for_port "127.0.0.1" 7001 "${API_PID}" "MAS Tune API"

        log "执行 MAS 推理..."
        "${CMD[@]}"

        log "清理 Tune API..."
        kill "${API_PID}" 2>/dev/null || true
        unset API_PID
        sleep 5

    elif [[ "${MODE}" == "local" ]]; then
        log "执行 Local 推理，模型: ${MODEL_ARG}"
        CMD+=(--model_path "${MODEL_ARG}")
        "${CMD[@]}"

    elif [[ "${MODE}" == "online" ]]; then
        log "执行 Online 推理，模型: ${MODEL_ARG}"
        CMD+=(--online_model "${MODEL_ARG}")
        if [[ -n "${API_URL}" ]]; then
            CMD+=(--api_url "${API_URL}")
        fi

        if [[ -n "${API_KEY}" ]]; then
            CMD+=(--api_key "${API_KEY}")
        fi
        "${CMD[@]}"
    fi
    

    log "--- [${MODEL_NAME}] 测试完成 ---"
    echo
done

log "所有配置运行完毕！"